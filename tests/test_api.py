...
def api_app():
    ...
    with mock_app.app_context():
        ...
        mock_db.session.add_all([user1, user2])
...
def test_user_roles_api(api_app, client):
    with api_app.app_context():
        ...
        mock_db.session.add(admin_role)

        # Create admin user
        admin = MockUser(username='api_admin', email='apiadmin@example.com')
        admin.set_password('adminpass')
        admin.roles.append(admin_role)
        mock_db.session.add(admin)
        mock_db.session.commit()

        # Add API endpoint for testing roles
        @api_app.route('/api/test/admin-check/<username>')
